...
$(document).ready(()=>{
  ...
  if(search_url === '/teacher/dashboard' || url_ary[3] === 'myclass'){
    $('.search_btn').on('click',function(){
      ...
      axios.get('https://monicare.herokuapp.com/api/search_student',{params:{serach_value: $('.search_value').val(), class: url_ary[3], class_id: url_ary[4]}})
  ...
  if (url_ary[1] === 'dashboard' && url_ary[4] === 'overview') {
    $('.search_btn_show').on('click', function() {
      ...
      if ($('.search').val() !== '' && date_style) {
        axios.get('https://monicare.herokuapp.com/api/search_dashboard',{params:{serach_value: $('.search').val(), child_id: url_ary[3]}})
            .then(function(response){
              if (response.status === 200) {
                return response.data
              } else {
  ...
  if (url_ary[1] === 'teacher' && url_ary[2] === 'dashboard' && url_ary[5] === 'overview') {
    $('.search_btn_show').on('click', function() {
      ...
      let date_style = /\d\d\d\d-\d\d-\d\d/.test($('.search').val())
      if ( $('.search').val() !== '' && date_style){
        axios.get('https://monicare.herokuapp.com/api/search_dashboard',{params:{serach_value: $('.search').val(), child_id: url_ary[4]}})
            .then(function(response){
              if (response.status === 200) {
                return response.data
              } else {
